...
for child in root:
	if (child.tag == "BugInstance"):
		#if not(("com.google" in child.find("Class").attrib['classname']) or ("com.apache" in child.find("Class").attrib['classname']) or ("android.support" in child.find("Class").attrib['classname'])):
		#if (any(substring in str(child.attrib['package']) for substring in BlackListArray)):
			...
			if (any(substring in child.find("Class").attrib['classname'] for substring in BlackListArray)):
				...
				for child1 in child:
					...
					if (child1.tag == 'LocalVariable'):
						if (child1.find('Message') != None):
							print(child1.find('Message').text, file=htmfile)
						else:
							pass
					if (child1.tag == 'LongMessage'):
						tv = child.find("LongMessage").text
				print("<b>Exact Problem: </b>", file=htmfile)
...
<script type="text/javascript">
      ...
      function drawChart() {
        ...
        var options = {
          title: 'Bugs by Type',
          chartArea:{right:25,top:50,width:"70%",height:"70%"},
          pieHole: 0.4
        };
...
<script type="text/javascript">
      ...
      function drawChart() {
        ...
        var options = {
          ...
          chartArea:{left:25,top:50,width:"70%",height:"70%"},
          pieHole: 0.4
        };
