<?php
	...
	class User{
		...
		function addInfo($email,$name,$contact,$audits_allowed,$account_type,$currentdate,$is_online,$active,$verification_code,$utm_source,$utm_medium,$utm_campaign, $ref){
			...
			mysql_query("INSERT INTO `user_plan_map`(`user_id`, `plan_id`, `transaction_date`,`payment_gateway_order_id`, `amount`, `for_months`) VALUES ('$this->user_id','0','$currentdate','0','0','0')") or die(mysql_error() . ' Errro is inserting user_plan_map');
		...
		function getAllAudits(){
			$result = mysql_query("SELECT `audit_id` FROM `audit_details` WHERE `user_id` = '$this->user_id' order by audit_id desc") or die("Unable to fetch all audit ".mysql_error());
			$toReturn = array();
		...
		function getRecentAudits($i){
			$result = mysql_query("SELECT `audit_id` FROM `audit_details` WHERE `user_id` = '$this->user_id' ORDER BY `audit_details`.`audit_id` Desc Limit $i") or die("Unable to fetch Recent audits ".mysql_error());
			$toReturn = array();
		...
		function isScanLimited($user_to_check = NULL){
			global $log;
			//echo json_encode($log);
			$log->LogDebug("inside isEligibleForFullScan",__FILE__,__LINE__);
			if(is_null($user_to_check))
			{
				$user_to_check = $this;
				$log->LogDebug("In isEligibleForFullScan - NULL passed as user will use this as user",__FILE__,__LINE__);
			}
			$log->LogDebug("In isEligibleForFullScan - current_plan of passed user with email id {$user_to_check->email} is ".json_encode($user_to_check->getCurrentPlanId()),__FILE__,__LINE__);
				
			if($user_to_check->getCurrentPlanId() !=0)
				return 0;
			else 
				return 1;
		...
		function sendActivationMail(){
			...
			<br /><br />
			Your FREE Trial for Appvigil has been created. To get started we are giving one audit for free. 
			<br /><br />
			...
			"<br /><br />
			and set your password to login into Appvigil.
			<br /><br />
			...
			Toshendra Sharma,<br />
			CEO, Wegilant";
			$mail_sub = "Welcome to Appvigil by Wegilant. Activate your Account now!";
		...
		function sendForgotPasswordMail(){
			...
			$mail_body = "Dear Appvigil User<br/><br/>
			...
			$mail_sub = "Appvigil Password Reset";
		$msgHeaders = 'MIME-Version: 1.0' . "\r\n";
				$msgHeaders .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
				$msgHeaders .= "From: $sendmail_from\r\n";
		...
		function sendReferralMail($email_list){
			...
			<br /><br />
			At Wegilant, we are building Appvigil, a cloud-based Android App security auditing tool.
			Your friend {$refererName}, has already tried Appvigil and would like to have you take a look
			at it.
			<br /><br />
			Hope you find our tool useful.
			<br /><br />
			<a href='http://{$_SERVER['SERVER_NAME']}$offset/#free_trial/?referral=$refererId' target='_blank'>Click here</a> to signup for Appvigil and get started.
			<br /><br />
			Thank you,
			<br />
			Team Appvigil @ Wegilant A SINE IIT Bombay Company";
		...
		function sendSubuserConfirmationMail($email){
			...
			if($subuser->getField('active')){
				...
				$mail_body = "Dear {$subuser_name},<br /><br />
					You have been added as subuser in Appvigil by your friend, {$parent_name}.
					<br /><br />
					Your friend {$parent_name}, is having a standard account and like to have you as a subuser under him.
					<br /><br />
					As subuser benefit, you will have unlimited audits of your android app by Appvigil. Hope you find our tool useful.
					<br /><br />
