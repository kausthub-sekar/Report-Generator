<?php
	...
	function submit_contact_us_request($name, $email, $message, $utm_source, $utm_campaign, $utm_medium){
		...
		//$message = nl2br("$message");
			global $support_email,$sendmail_from;
			$mail_body = "Hello Admin,
			<br /><br />
			I am $name and had this message/query for you:<br><br> 
			
			Message: $message<br>
			
			<br>
			Please respond to my message/query asap.
			<br><br>
			...
			if(mail($support_email, $mail_sub, $mail_body, $msgHeaders)){
				...
				if($result)
				{
				$log->LogFatal("Contact us request submitted & but email could not be send successfully at $support_email with $message",__FILE__,__LINE__);
				}
				else
				{ 
				 $log->LogFatal("Contact us request could not be submitted & email could not be send at $support_email",__FILE__,__LINE__);
				}
			}
