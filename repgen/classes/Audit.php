...
class Audit{
		...
		function __construct($user_id, $audit_date_time, $app_name, $app_size_in_mb, $app_absolute_location, $audit_status, $isNew){
			...
			if($isNew===true){
				...
				$this->p2_count=null;
				$this->p3_count=null;
				$this->BugsByCategoriesChartData=null;
		...
		function fetchInfo(){
			...
			$result = mysql_query("SELECT * FROM audit_details where `audit_id` = '{$this->audit_id}'");
				$this->info = mysql_fetch_assoc($result);
				 
				//refresh all the counts
				/*
				$this->p1_count = $this->getP1Count();
				$this->p2_count = $this->getP2Count();
				$this->p3_count = $this->getP3Count();
				$this->BugsByCategoriesChartData = $this->getBugsByCategoriesChartData(); 
				*/
		}
		...
		function getBugsByCategoriesChartData(){
			...
			if(is_null($this->BugsByCategoriesChartData))
			{
			...
			$this->BugsByCategoriesChartData = $json_bugs_by_categories;
			return  $json_bugs_by_categories;
			}
			else
				return $this->BugsByCategoriesChartData;
		}
		
		function getTotalBugsCount(){
			return $this->getP1Count() + $this->getP2Count() + $this->getP3Count();
		}
